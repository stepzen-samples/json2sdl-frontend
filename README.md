<p align="center">
  <a href="https://json2sdl.com">
    <img src="./assets/images/icon.png" height="128">
    <h1 align="center">JSON2SDL.com</h1>
  </a>
</p>

<p align="center">
  <a aria-label="StepZen logo" href="https://stepzen.com">
    <img src="https://img.shields.io/badge/-MADE%20BY%20STEPZEN-%23193460?style=for-the-badge&logo=data:image/png;base64,&labelColor=#193460">
  </a>
  <a aria-label="License" href="https://github.com/stepzen-dev/json2sdl-frontend/blob/main/contributing.md">
    <img alt="" src="https://img.shields.io/npm/l/stepzen.svg?style=for-the-badge&labelColor=000000">
  </a>
</p>

## What is JSON2SDL.com?

JSON2SDL.com is a frontend to the StepZen JSON2SDL introspection tool. The frontend application stringifies a GraphQL Query with the provided JSON of the user and returns the GraphQL SDL (schema definition language) in the code editor.

## Dependencies

[Apline.js](https://alpinejs.dev/) handles the form input, fetches the GraphQL endpoint, and returns the SDL (schema definition language).

[clipboard.js](https://clipboardjs.com/) provides users the option to copy the newly generate SDL (schema definition language).  

## Community

The [StepZen discord](https://discord.com/invite/9k2VdPn2FR) is where you can ask questions and voice ideas related to JSON2SDL or the GraphQL schemas created by it.

## Contributing

Please see our [contributing.md](/contributing.md).
